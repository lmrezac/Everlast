...
public class Scope {
	...
	public static final Function substring, sqrt, type, isNumber, index,
			tostring, length;
	public static final Struct player;
	public static Map map;
	public static final FacingStruct facing_N, facing_NE, facing_E, facing_SE,
			facing_S, facing_SW, facing_W, facing_NW;
	public static final StructTemplate player_type, map_type, facing_type, tile_type;
	...
	static {
		...
		facing_type
				.setFinalVar("north",
						(facing_N = new FacingStruct(Facing.N)).getIdStr(),
						facing_type);
		facing_type.setFinalVar("northeast", (facing_NE = new FacingStruct(
				Facing.NE)).getIdStr(), facing_type);
		facing_type
				.setFinalVar("east",
						(facing_E = new FacingStruct(Facing.E)).getIdStr(),
						facing_type);
		facing_type.setFinalVar("southeast", (facing_SE = new FacingStruct(
				Facing.SE)).getIdStr(), facing_type);
		facing_type
				.setFinalVar("south",
						(facing_S = new FacingStruct(Facing.S)).getIdStr(),
						facing_type);
		facing_type.setFinalVar("southwest", (facing_SW = new FacingStruct(
				Facing.SW)).getIdStr(), facing_type);
		facing_type
				.setFinalVar("west",
						(facing_W = new FacingStruct(Facing.W)).getIdStr(),
						facing_type);
		facing_type.setFinalVar("northwest", (facing_NW = new FacingStruct(
				Facing.NW)).getIdStr(), facing_type);
		player_type = new StructTemplate("Player", false);
		tile_type = new StructTemplate("Tile",false);
		tile_type.setFinalVar("getTileFromId", new Function(null,null,false){
		...
		player = new Struct(player_type, Arrays.asList("x", "y", "map",
				"facing")) {
			private final Function toString = new Function(null, null, false) {
				...
				@Override
				public String call(Scope script, List<String> values) {
					com.ombda.entities.Player p = Panel.getInstance()
							.getPlayer();
					return "player@(" + Scope.toString(p.x) + ","
							+ Scope.toString(p.y) + ")["
			...
			public String getVar(String varname, Scope scopeIn) {
				if (varname.equals("x"))
					return Integer.toString((int) Panel.getInstance()
							.getPlayer().x);
					...
					return Panel.getInstance().getPlayer().getDirection()
							.getStruct().getIdStr();
				else if (varname.equals("tostring"))
					return toString.getIdStr();
				else
					throw new VarNotExists("Variable " + varname
							+ " does not exist for player.");
			}

			@Override
			public void setVar(String varname, String value, boolean isfinal,
					Scope scopeIn) {
				if (varname.equals("facing")) {
					if (!value.startsWith(Script.REF))
						throw new RuntimeException(
								"Expected facing value, got : " + value);
					...
					if (!value.startsWith(Script.REF)) {
						...
						if (!(scope instanceof com.ombda.Map))
							throw new RuntimeException(
									"Expected map value, got : " + value);
						Panel.getInstance().getPlayer()
								.setMap((com.ombda.Map) scope);
						Scope.map = (com.ombda.Map) scope;
					}
				} else
					throw new RuntimeException("Variable " + varname
							+ " doesn't exist in player");
			}
		};
		...
		length = new Function(null, null, false) {
			...
			public String call(Scope script, List<String> values) {
				...
				if (value.startsWith(Script.REF))
					throw new RuntimeException(
							"Invalid value passed for function length, expected a string.");
			...
			public String call(Scope script, List<String> values) {
				...
				if (value.startsWith(Script.REF)) {
					...
					if (scope instanceof Function)
						...
						return ((ListScope) scope).variadic() ? "List"
								: "array";
		...
		index = new Function(null, null, false) {
			...
			public String call(Scope script, List<String> values) {
				...
				if (!value.startsWith(Script.REF))
					throw new RuntimeException(
							"Expected a scoped value before []");
				Scope scope = Scope.getId(value);
		...
		globalScope = new Scope(true) {
			@Override
			public void setVar(String name, String value, boolean finalVar,
					Scope scopeIn) {
	...
	public Scope(boolean functions) {
		...
		if (functions) {
			...
			if (Panel.getInstance().getPlayer() != null
					&& Panel.getInstance().getPlayer().getMap() != null)
				setVar("map",
	...
	protected static boolean isReservedWord(String s) {
		return s.equals("class") || s.equals("set") || s.equals("script")
				|| s.equals("msg") || s.equals("struct") || s.equals("scope")
				|| s.equals("function") || s.equals("end") || s.equals("array")
				|| s.equals("List") || s.equals("not") || s.equals("and")
				|| s.equals("or") || s.equals("=") || s.equals("not=")
				|| s.equals("if") || s.equals("else") || s.equals("delete")
				|| s.equals("while") || s.equals("then") || s.equals("do")
				|| s.equals("define") || s.equals("assert");
	}
	...
	public void setVar(String varname, String value, Scope scope) {
		setVar(varname, value, false, scope);
	}

	public void setVar(String varname, String value, boolean isfinal,
			Scope scopeIn) {
		if (isReservedWord(varname.trim()))
			throw new RuntimeException("Cannot set var of name " + varname
					+ ", it is a reserved word.");
		if (varname.equals("map")) {
			if (isfinal)
				throw new RuntimeException("Cannot set final var map");
			String mapname = value;
			if (value.startsWith(Script.REF)) {
				Scope scope = Scope.getId(value);
				if (!(scope instanceof Map))
					throw new RuntimeException(
							"Cannot set value of map to non-map object!");
		...
		if (!varname.startsWith("class ") && !require_class
				&& varname.contains("."))
		...
		if (varname.startsWith("class ")) {
			...
			if (i == -1) {
				if (finalvars.containsKey(varname) && finalvars.get(varname))
					throw new RuntimeException("Cannot assign variable "
							+ varname + ", it is final.");
				if (!value.startsWith(Script.REF))
					throw new RuntimeException("Cannot assign subscope "
							+ varname + " to a non-scope value!");
				...
				// throw new
				// RuntimeException("Sub-scope "+varname+" does not exist!");
			} else {
				String var = varname.substring(i + 1);
				varname = varname.substring(0, i);
				String val = vars.get(varname);
				if (val == null)
					throw new RuntimeException("Subscope of name " + varname
							+ " doesn't exist!");
				...
				if (scope instanceof Function)
					throw new RuntimeException(
							"Cannot set variable of a function.");
				if (!var.startsWith("class ") && !require_class
						&& var.contains("."))
					var = "class " + var;
			...
			if (finalvars.containsKey(varname) && finalvars.get(varname))
				throw new RuntimeException("Cannot assign variable " + varname
						+ ", it is final.");
			if (value.startsWith(Script.REF))
				getId(value);
	...
	public String getVar(String varname, Scope scopeIn) {
		if (varname.startsWith("npc ") && varname.contains(".")) {
			...
			if (varname.equals("xy") || varname.equals("pos")) {
				...
				xy.setVar("0", Script.toString(npc.x / (Tile.SIZE / 16)),
						scopeIn);
				xy.setVar("1", Script.toString(npc.y / (Tile.SIZE / 16)),
						scopeIn);
				return xy.getIdStr();
				...
				ListScope xy = new ListScope(2);
				xy.setVar("0", Script.toString(npc.destX / (Tile.SIZE / 16)),
						scopeIn);
				xy.setVar("1", Script.toString(npc.destY / (Tile.SIZE / 16)),
						scopeIn);
				return xy.getIdStr();
			} else if (varname.equals("facing")) {
				...
				throw new RuntimeException("Cannot set variable " + varname
						+ " in npc");
		}
		if (varname.contains("operator"))
			varname = varname.replaceAll("operator(?! )", "operator ");
		...
		if (varname.startsWith("class ")) {
			...
			if (i == -1) {
				...
				if (value == null || !value.startsWith(Script.REF))
					throw new VarNotExists("Scoped variable " + varname
							+ " does not exist.");
				scope = getId(value);
				if (scope == null)
					throw new VarNotExists("Subscope of name " + varname
							+ " doesn't exist.");
				...
				if (scope instanceof Function)
					throw new RuntimeException(
							"Cannot get variable from scope of function.");
	...
	public void deleteVar(String varname) {
		...
		if (varname.startsWith("class ")) {
			...
			if (i == -1) {
				...
				if (scope == null)
					return;// throw new
							// VarNotExists("Subscope of name "+varname+" doesn't exist.");
				...
				if (scope instanceof Function)
					throw new RuntimeException(
							"Cannot get/delete variable from scope of function.");
				if (!var.startsWith("class ") && !require_class
						&& var.contains("."))
					var = "class " + var;
				scope.deleteVar(var);
	...
	public String evalVars(String line) {
		...
		if (!isString(str) && !require_brackets && !str.matches("-?((\\d+(\\.\\d+)?)|(\\d*\\.\\d+))")) {
			if (currentScope.vars.containsKey(str) || ((!str.startsWith("npc ") && 
				(str.startsWith("class ") || (!require_class && str.contains("."))) 
				&& !str.contains("${")))) {
				
				if (str.contains(".") && !str.startsWith("class ") && !require_class && !str.startsWith("npc ") )
					str = "class " + str;
		...
		while ((i = str.indexOf("${")) != -1) {
			...
			if (j == -1)
				throw new RuntimeException("Unclosed variable at index " + i
						+ " in string " + line);
			String varname = str.substring(i + 2, j);
			str = str.substring(0, i)
					+ currentScope.getVar(varname, currentScope)
					+ (j + 1 < str.length() ? str.substring(j + 1) : "");
		}
		return str;
	}
	...
	public void evalArgs(List<String> args) {
		...
		for (int i = 0; i < args.size(); i++) {
			...
			if (!arg.contains("${")
					...
					&& !require_class
					&& arg.contains(".")
					&& ((i > 0 && !args.get(i - 1).equals("class") && !args
							.get(i - 1).equals("npc")) || i == 0)) {
				args.add(i, "class");
			}
		}
		for (int i = 0; i < args.size(); i++) {
			String arg = args.get(i);
			if (!require_brackets
					&& (arg.endsWith("class") || arg.endsWith("operator"))) {
				if (i + 1 >= args.size())
					throw new RuntimeException(
							"Keyword class expects a variable name!");
				arg += " " + evalString(args.remove(i + 1));
				while (arg.endsWith("class") || arg.endsWith("operator")) {
					if (i + 1 >= args.size())
						throw new RuntimeException(
								"Keyword class/operator expects a variable/operator name!");
					arg += " " + evalString(args.remove(i + 1));
				}
				try {
				...
				args.set(i, currentScope.getVar(arg + " " + args.remove(i + 1),
						currentScope));
			}
		}
		evalMath(args);
	...
	public void evalMath(List<String> args) {
		...
		while (!args.equals(lastPass)) {
			for (index = args.size() - 1; index >= 0; index--) {
				...
				if (arg.startsWith(Script.REF)) {
					if ((index != 0 && !args.get(index - 1).equals("function"))
							|| index == 0) {
						...
						if (scope instanceof Function) {
							...
							args.set(index,
									func.call(getCurrentScope(), values));
						}
					} else {
						...
						if (!value.startsWith(Script.REF))
							throw new RuntimeException(
									"Value after function declaration MUST be a scoped value! Got: "
											+ value);
						Scope scope = Scope.getId(value);
						if (!(scope instanceof Function))
							throw new RuntimeException(
									"Value after function declaration MUST be a function value.");
						args.set(index, scope.getIdStr());
					}
				} else if (arg.equals("new")) {
					if (index >= args.size() - 1)
						throw new RuntimeException(
								"Expected type name after new declaration");
					String name = evalString(args.remove(index + 1));
					if (name.equals("scope")) {
						...
						if (index + 1 >= args.size())
							throw new RuntimeException(
									"Expected size after new array declaration.");
						...
						if (!(scope instanceof StructTemplate))
							throw new RuntimeException(
									"Cannot create new object from non-struct template");
					...
					if (index >= args.size() - 2)
						throw new RuntimeException(
								"Expected parenthesis after collect");
					args.remove(index);
					List<String> values = new ArrayList<>();
					...
					for (int j = 0; j < values.size(); j++)
						list.setVar(toString(j), values.get(j),
								getCurrentScope());
				...
				if (arg.equals("^")) {
					...
					if (!string)
						try {
							...
							string = testOpOverride(args, index, arg1, arg2,
									"operator ^");
						}
					if (string) {
			...
			for (index = 1; index < args.size() - 1; index++) {
				...
				if (arg.equals("*") || arg.equals("/")) {
					...
					if (!string)
						try {
							double x = parseDouble(evalVars(arg1)), y = parseDouble(evalVars(arg2));
							args.set(index, toString(arg.equals("*") ? (x * y)
									: (x / y)));
						} catch (NumberFormatException e) {
							string = testOpOverride(args, index, arg1, arg2,
									"operator " + arg);
				...
				if (arg.equals("+") || arg.equals("-")) {
					...
					if (!string) {
						try {
							double x = parseDouble(evalVars(arg1)), y = parseDouble(evalVars(arg2));
							args.set(index,
									Script.toString(arg.equals("+") ? (x + y)
											: (x - y)));
						} catch (NumberFormatException e) {
							string = testOpOverride(args, index, arg1, arg2,
									"operator " + arg);
				...
				if (arg.equals("=") || arg.equals("not=")) {
					...
					if (!string)
						try {
							...
							if(arg1_.endsWith("t")) x /= Tile.SIZE/16;
							if(arg2_.endsWith("t")) y /= Tile.SIZE/16;
							args.set(index, (arg.equals("not=") ? !(x == y)
									: (x == y)) ? "1" : "0");
						} catch (NumberFormatException e) {
							string = testOpOverride(args, index, arg1, arg2,
									"operator " + arg);
							if (string) {
								string = testOpOverride(args, index, arg1,
										arg2, "operator =");
								if (string) {
									string = testOpOverride(args, index, arg1,
											arg2, "equals");
					...
					if (string)
						args.set(
								index,
								(arg.equals("not=") ? !arg1.equals(arg2) : arg1
										.equals(arg2)) ? "1" : "0");
			...
			for (index = 1; index < args.size() - 1; index++) {
				...
				if (arg.equals("and")) {
					...
					if (arg1.startsWith(Script.REF)
							|| arg2.startsWith(Script.REF))
						no_override = testOpOverride(args, index, arg1, arg2,
								"operator and");
					if (no_override) {
						if (arg1.endsWith(".0"))
							arg1 = arg1.substring(0, arg1.length() - 2);
						...
						args.set(index,
								arg1.equals("0") ? "0" : arg2.equals("0") ? "0"
										: arg2);
					}
				}
			} // and
	...
	private boolean testOpOverride(List<String> args, int index, String arg1_,
			String arg2_, String op) {
		...
		if (arg1.startsWith(Script.REF)) {
			...
			try {
				if (scope instanceof Struct
						&& (var = scope.getVar(op, this))
								.startsWith(Script.REF)
						&& Scope.getId(var) instanceof Function
						&& (func = (Function) Scope.getId(var)).args_length() == 1)
			...
			if (op.equals("equals") || op.equals("operator =")
					|| op.equals("operator not="))
	...
	private boolean testOpBool(List<String> args, int index, String arg1) {
		...
		if (arg1.startsWith(Script.REF)) {
			...
			try {
				if (scope instanceof Struct
						&& (var = scope.getVar("operator bool", this))
								.startsWith(Script.REF)
						&& Scope.getId(var) instanceof Function
						&& (func = (Function) Scope.getId(var)).args_length() == 0)
	...
	public String toString() {
		return "[id=" + id + ", vars=" + vars.toString() + "]";
	}
