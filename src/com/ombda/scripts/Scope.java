	...
	static{
		...
		tile_type.setFinalVar("getTileFromId", new Function(null, null, false){
			...
			public String call(Scope script, List<String> values){
				...
				int i = Script.parseInt(str);
				return Tile.getTile(i).getIdStr();
			}
		}.getIdStr(), tile_type);
		...
		player = new Struct(player_type, Arrays.asList("x", "y", "map", "facing")){
			...
			public String getVar(String varname, Scope scopeIn){
				if(varname.equals("x"))
					...
					throw new VarNotExists("Variable " + varname + " does not exist for player.");
			}
			...
			public void setVar(String varname, String value, boolean isfinal, Scope scopeIn){
				if(varname.equals("facing")){
					...
					throw new RuntimeException("Variable " + varname + " doesn't exist in player");
			}
		};
	...
	public void setVar(String varname, String value, boolean isfinal, Scope scopeIn){
		...
		if(varname.startsWith("class ")){
			...
			if(i == -1){
				...
				if(!var.startsWith("class ") && !require_class && var.indexOf(".") > 0)
					var = "class " + var;
	...
	public String getVar(String varname, Scope scopeIn){
		...
		if(varname.startsWith("class ")){
			...
			if(i == -1){
				...
				if(varname.equals("map")){
					if(map == null){
						if(Panel.getInstance().getPlayer() != null && Panel.getInstance().getPlayer().getMap() != null)
							value = (map = Panel.getInstance().getPlayer().getMap()).getIdStr();
						else
							value = "map";
					}else
						value = map.getIdStr();
				...
				if(!var.startsWith("class ") && var.indexOf(".") > 0)
					var = "class " + var;
